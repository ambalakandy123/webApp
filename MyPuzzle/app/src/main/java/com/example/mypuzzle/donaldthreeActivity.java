...
public class donaldthreeActivity extends AppCompatActivity {

    private final int NO_OF_PIECES = 9;
    private final String BUTTON_NAME_PREFIX = "btnd";
    int time_to_solve_puzzle = 1;
    private MediaPlayer mp;

    //an array of buttons
    Button[] btnd = new Button[NO_OF_PIECES];
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_donaldthree);
       ...
       // test_db();
        ...
        for (int i = 0; i < NO_OF_PIECES; i++) {
            btnd[i] = (Button) findViewById(this.getResources().getIdentifier(
    ...
    public void play_game(int perusal_time_by_seconds, String image_name) {
        ...
        for (int i = 0; i < NO_OF_PIECES; i++) {
            btnd[i].setBackgroundResource(correct_id_seq[i]);
        }

        for (int i = 0; i < NO_OF_PIECES; i++) {
            btnd[i].setClickable(false);
        }
    ...
    public void make_puzzle_with_time_tick() {
        ...
        for (int i = 0; i < NO_OF_PIECES; i++) {
            btnd[i].setBackgroundResource(rand_id_seq[i]);
        }
    ...
    public void on_click_image(View v) {
        ...
        //value of temp similar to com.jc165984.puzzle:id/btnd0
        String temp_str = button.getResources().getResourceName(button.getId());

        // "id/" + BUTTON_NAME_PREFIX = "id/btnd"
        int id_pos = temp_str.indexOf("id/" + BUTTON_NAME_PREFIX);

        //get the button's index.For example, id/btnd0 has index 0
        ...
        if (num_of_clicks == 1) {
            ...
            if (Arrays.equals(correct_id_seq, rand_id_seq)) {
                ...
                //Plays the mediaplayer after the game
                if (mp != null) mp.release();
                mp = MediaPlayer.create(getApplicationContext(), R.raw.d);
                mp.start();
                //display the Image name after game successful
