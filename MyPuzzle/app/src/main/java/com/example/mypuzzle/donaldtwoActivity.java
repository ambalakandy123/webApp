...
public class donaldtwoActivity extends AppCompatActivity {

    private final int NO_OF_PIECES = 4;
    private final String BUTTON_NAME_PREFIX = "btndd";
    int time_to_solve_puzzle = 1;
    private MediaPlayer mp;

    //an array of buttons
    Button[] btndd = new Button[NO_OF_PIECES];
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_donaldtwo);
        ...
        for (int i = 0; i < NO_OF_PIECES; i++) {
            btndd[i] = (Button) findViewById(this.getResources().getIdentifier(
    ...
    public void play_game(int perusal_time_by_seconds, String image_name) {
        ...
        for (int i = 0; i < NO_OF_PIECES; i++) {
            btndd[i].setBackgroundResource(correct_id_seq[i]);
        }

        for (int i = 0; i < NO_OF_PIECES; i++) {
            btndd[i].setClickable(false);
        }
    ...
    public void make_puzzle_with_time_tick() {
        ...
        for (int i = 0; i < NO_OF_PIECES; i++) {
            btndd[i].setBackgroundResource(rand_id_seq[i]);
        }
    ...
    public void on_click_image(View v) {
        ...
        //value of temp similar to com.jc165984.puzzle:id/btddm0
        String temp_str = button.getResources().getResourceName(button.getId());

        // "id/" + BUTTON_NAME_PREFIX = "id/btddm"
        int id_pos = temp_str.indexOf("id/" + BUTTON_NAME_PREFIX);

        //get the button's index.For example, id/btdd0 has index 0
        ...
        if (num_of_clicks == 1) {
            ...
            if (Arrays.equals(correct_id_seq, rand_id_seq)) {
                ...
                //The recording will play here after the game
                if (mp != null) mp.release();
                mp = MediaPlayer.create(getApplicationContext(), R.raw.d);
                mp.start();
                //shows the toast message after the game
                CharSequence text = "Donald Duck " + time_to_solve_puzzle + " Seconds Congratulations";
    ...
    private void test_db() {
        ...
        myDB.insertPlayer(MessageText, 70, 1, "dnd");
