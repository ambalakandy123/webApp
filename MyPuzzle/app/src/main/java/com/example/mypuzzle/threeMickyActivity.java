...
public class threeMickyActivity extends AppCompatActivity {
    private final int NO_OF_PIECES = 9;
    private final String BUTTON_NAME_PREFIX = "btnmi";
    int time_to_solve_puzzle = 1;
    private MediaPlayer mp;

    //an array of buttons
    Button[] btnmi = new Button[NO_OF_PIECES];
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_three_micky);
        ...
        for (int i = 0; i < NO_OF_PIECES; i++) {
            btnmi[i] = (Button) findViewById(this.getResources().getIdentifier(
    ...
    public void play_game(int perusal_time_by_seconds, String image_name) {
        ...
        for (int i = 0; i < NO_OF_PIECES; i++) {
            btnmi[i].setBackgroundResource(correct_id_seq[i]);
        }

        for (int i = 0; i < NO_OF_PIECES; i++) {
            btnmi[i].setClickable(false);
        }
    ...
    public void make_puzzle_with_time_tick() {
        ...
        for (int i = 0; i < NO_OF_PIECES; i++) {
            btnmi[i].setBackgroundResource(rand_id_seq[i]);
        }
    ...
    public void on_click_image(View v) {
        ...
        //value of temp similar to com.jc165984.puzzle:id/btbtnmi0
        String temp_str = button.getResources().getResourceName(button.getId());

        // "id/" + BUTTON_NAME_PREFIX = "id/btnmi"
        int id_pos = temp_str.indexOf("id/" + BUTTON_NAME_PREFIX);

        //get the button's index.For example, id/btnmi has index 0
        ...
        if (num_of_clicks == 1) {
            ...
            if (Arrays.equals(correct_id_seq, rand_id_seq)) {
                ...
                for (int i = 0; i < NO_OF_PIECES; i++) {
                    btnmi[i].setClickable(false);
                    CharSequence text = "congratulations you Won in : " + time_to_solve_puzzle + "Seconds Try Again to Improve the time";
                    int duration = Toast.LENGTH_LONG;
                    Toast toast = Toast.makeText(this, text, duration);
                    toast.show();
                }
                //play the music after the succeful button order
                if (mp != null) mp.release();
                mp = MediaPlayer.create(getApplicationContext(), R.raw.h);
                mp.start();
                //display message after successfull complition of the game
                CharSequence text = "Micky Mouse YOU WON IN " + time_to_solve_puzzle + " Seconds!";
    ...
    private void test_db() {
        ...
        myDB.insertPlayer(MessageText, 40, 1, "c");
