...
public class threepopActivity extends AppCompatActivity {
    private final int NO_OF_PIECES = 9;
    private final String BUTTON_NAME_PREFIX = "btnp";
    int time_to_solve_puzzle = 1;
    private MediaPlayer mp;
    //an array of buttons
    Button[] btnp = new Button[NO_OF_PIECES];
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_threepop);
        ...
        for(int i = 0; i < NO_OF_PIECES; i++) {
            btnp[i] = (Button) findViewById(this.getResources().getIdentifier(
    ...
    public void play_game(int perusal_time_by_seconds, String image_name) {
        ...
        for(int i = 0; i < NO_OF_PIECES; i++){
            btnp[i].setBackgroundResource(correct_id_seq[i]);
        }

        for(int i = 0; i < NO_OF_PIECES; i++){
            btnp[i].setClickable(false);
        }
    ...
    public void make_puzzle_with_time_tick(){
        ...
        for(int i = 0; i < NO_OF_PIECES; i++){
            btnp[i].setBackgroundResource(rand_id_seq[i]);
        }
    ...
    public void on_click_image(View v){
        ...
        //value of temp similar to com.jc165984.puzzle:id/btnp0
        String temp_str = button.getResources().getResourceName(button.getId());

        // "id/" + BUTTON_NAME_PREFIX = "id/btnp"
        int id_pos = temp_str.indexOf("id/" + BUTTON_NAME_PREFIX);

        //get the button's index.For example, id/btnp has index 0
        ...
        if (num_of_clicks == 1) {
            ...
            if (Arrays.equals(correct_id_seq, rand_id_seq)) {
                ...
                mp.start();
                CharSequence text = "Popayee congratulations you Won in : "+time_to_solve_puzzle+ "Seconds Try Again to Improve the time"; int duration = Toast.LENGTH_LONG;
